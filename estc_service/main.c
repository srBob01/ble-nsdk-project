...
static color_rgb_t m_color_value = {255, 1, 1};
...
void update_color(const ble_evt_t * evt)
{
    ...
    if (evt_write->handle == m_estc_service.color_write_handle.value_handle)
    {
        ...
        pwm_set_rgb_color_ble(&m_color_value);

        estc_ble_char_color_notify_update(&m_estc_service, m_color_value);

        save_to_flash(&m_color_value, &fstorage, &color_value_flash);
    }
}
...
int main(void)
{
    ...
    init_flash(&fstorage);
    read_from_flash(&fstorage, &color_value_flash, &m_color_value);

    // Start execution.
    NRF_LOG_INFO("ESTC GATT service example started");
    application_timers_start();

    pwm_init_ble(&m_color_value);
