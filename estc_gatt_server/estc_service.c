...
static ret_code_t estc_ble_add_notify_characteristics(ble_estc_service_t *service)
{
    ...
    error_code = sd_ble_gatts_characteristic_add(service->service_handle, &char_md, &attr_char_value, &service->notify_char_handle);
    APP_ERROR_CHECK(error_code);
...
static ret_code_t estc_ble_add_indicate_characteristics(ble_estc_service_t *service)
{
    ...
    error_code = sd_ble_gatts_characteristic_add(service->service_handle, &char_md, &attr_char_value, &service->indication_char_handle);
    APP_ERROR_CHECK(error_code);
...
ret_code_t estc_ble_char_notify_value_update(ble_estc_service_t *service, uint8_t value)
{
    ...
    error_code = estc_ble_char_value_update(service->connection_handle, service->notify_char_handle.value_handle, BLE_GATT_HVX_NOTIFICATION, value, sizeof(uint8_t));
...
ret_code_t estc_ble_char_indicate_value_update(ble_estc_service_t *service, uint8_t value)
{
    ...
    error_code = estc_ble_char_value_update(service->connection_handle, service->indication_char_handle.value_handle, BLE_GATT_HVX_INDICATION, value, sizeof(uint8_t));
